  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      tasks.forEach(task => {
        ...
        // Only notify if user is owner or creator
        const isOwner = task.owner === auth.currentUser?.email;
        const isCreator = task.creatorId === auth.currentUser?.uid;
        ...
        task.reminders.forEach((r, idx) => {
          ...
          if (reminderTime <= now && !r.triggered) {
            ...
            updateDoc(taskRef, { reminders: updatedReminders, updatedAt: new Date().toISOString() })
