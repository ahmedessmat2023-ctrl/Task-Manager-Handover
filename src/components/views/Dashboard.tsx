...
export default function Dashboard({ tasks, handovers, offices, stats }: DashboardProps) {
  ...
  return (
    <div className="space-y-10 pb-20">
        ...
        <div className="lg:col-span-2 glass-card relative overflow-hidden group border-none bg-gradient-to-br from-ink to-slate-900 text-white p-10">
          <div className="relative z-10">
            ...
            <div className="flex gap-4">
              <button className="px-8 py-3 bg-citrus text-ink rounded-xl font-black text-xs uppercase tracking-widest hover:scale-[1.05] transition-all shadow-xl shadow-citrus/20">
                Action Risks
              </button>
              <button className="px-8 py-3 bg-white/10 hover:bg-white/20 border border-white/10 text-white rounded-xl font-black text-xs uppercase tracking-widest transition-all">
                Generate Brief
              </button>
            </div>
          </div>
      ...
      <section className="grid grid-cols-2 md:grid-cols-5 gap-6">
        {[
          { label: 'Open Outcomes', val: stats.openCount, note: 'Daily pending', icon: CheckSquare },
          { label: 'Critical Risks', val: stats.riskCount, note: 'SLA pressure', icon: AlertCircle, color: 'text-red-500' },
          { label: 'Carry-over', val: stats.carryCount, note: 'Multi-shift', icon: RefreshCw, color: 'text-citrus' },
          { label: 'Handovers', val: stats.handoverCount, note: 'Active sync', icon: Globe, color: 'text-blue-500' },
          { label: 'Acknowledged', val: acknowledgedCount, note: 'Confirmed received', icon: CheckCircle, color: 'text-green-500' },
        ].map((kpi, i) => (
          <div key={i} className="glass-card p-6 border-dawn hover:border-citrus/20 transition-all group">
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        <div className="xl:col-span-2 space-y-6">
          <div className="flex items-center justify-between px-2">
            ...
            <button className="text-[10px] font-black uppercase tracking-widest text-muted hover:text-ink transition-colors border border-dawn px-4 py-1.5 rounded-lg">Audit All Risks</button>
          </div>
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-dawn">
                {highRiskTasks.map((task) => (
                   <tr key={task.id} className="group hover:bg-stone/30 transition-colors">
        ...
        <div className="space-y-6">
          ...
          <div className="glass-card p-6 space-y-8 relative before:absolute before:left-10 before:top-10 before:bottom-10 before:w-[1px] before:bg-dawn">
            {handovers.slice(0, 5).map((ho, i) => (
              <div key={ho.id} className="relative pl-12">
      ...
      <section className="space-y-6">
        <div className="flex items-center justify-between px-2">
          ...
          <button className="text-[10px] font-black uppercase tracking-widest text-muted hover:text-ink transition-colors">Global Connectivity</button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {offices.map(office => {
            ...
            return (
              <div key={office.id} className="glass-card p-6 border-dawn hover:border-citrus/20 hover:shadow-lg transition-all group overflow-hidden">
