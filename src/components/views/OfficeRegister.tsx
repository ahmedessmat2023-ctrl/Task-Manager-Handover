...
export default function OfficeRegister({ offices, tasks }: OfficeRegisterProps) {
  ...
  const handleSave = async () => {
    ...
    if (editingOffice) {
      const ref = doc(db, 'offices', editingOffice.id);
      await updateDoc(ref, form);
      await logAction(ActionType.OFFICE_UPDATE, { name: form.name });
    } else {
      await addDoc(collection(db, 'offices'), form);
      await logAction(ActionType.OFFICE_REGISTER, { name: form.name });
    }
  ...
  const handleDelete = async (id: string, name: string) => {
    if (confirm(`Delete hub ${name}?`)) {
      ...
      await logAction(ActionType.OFFICE_DELETE, { name });
    }
  };
